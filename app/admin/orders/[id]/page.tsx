...
import CancelButtom from "./_componants/CancelButtom";
import OrderTimeline from "./_componants/OrderTimeline";
    ...
    <div className="max-w-2xl mx-auto p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex justify-center gap-3">
          <span
            className={`px-3 py-1 rounded-full text-sm font-semibold ${STATUS_STYLE[order.status]}`}
          >
            {order.status.replace(/_/g, " ")}
          </span>
          {order.status === "PENDING" && <CancelButtom orderId={order.id} />}
        </div>
      </div>
      ...
      <div className="border rounded-xl p-4 space-y-1 text-sm">
        ...
        <p>
          <span className="text-muted-foreground">Payment:</span> 🛵 Pay on
          Delivery
        </p>
        <p>
        ...
        {order.products.map((op) => (
          ...
          >
            ...
            <p className="font-medium">
              ${(op.product.basePrice * op.quantity).toFixed(2)}
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="border rounded-xl p-5">
        <p className="font-semibold text-base mb-4">Order status</p>
